...
int main() {
  ...
  setlocale(LC_ALL, "");
  auto r =
      expected<int, localerror>{3}  // 正常値3のexpectedを作成
          .and_then([](auto n) -> expected<vector<int>, localerror> {
            // 整数値をn回入力した結果を返すラムダ関数
            vector<int> v;
            v.reserve(n);
            for (int i = 0; i < n; ++i) {
              auto r =
                  expected<nullptr_t, localerror>(nullptr)
                      .and_then([](auto _) -> expected<string, localerror> {
                        // 標準入力から1行読んで返す
                        string s;
                        if (getline(cin, s)) {
                          // 1行分の文字列を返す
                      ...
                      .and_then([](auto s) -> expected<int, localerror> {
                        try {
                          // 文字列sをintにして返す
                          return {stoi(s)};
          ...
          .transform([](auto v) {  // 正常値のみ実行し、正常値を変換
            // int値ベクタを合計して返す
            int sum = 0;
            for (auto e : v) sum += e;
            return sum;
          })
          .transform_error([](auto e) -> wstring {
            // エラーのみ実行し、enumエラー値をエラー文字列に変換
            switch (e) {
